...
make_helper(concat(adc_i2rm_, SUFFIX)) {
	...
	if (m.mod == 3){ 
		...
		DATA_TYPE res = REG(m.R_M) + imm + eflags.CF;
		REG(m.R_M) = res;
		...
		DATA_TYPE res = MEM_R(addr) + imm + eflags.CF;
		MEM_W(addr,res);
...
make_helper(concat(adc_r2rm_, SUFFIX)) {
	...
	m.val = instr_fetch(eip + 1, 1);
	if (m.mod == 3){
		DATA_TYPE res = REG(m.R_M) + REG(m.reg) + eflags.CF;
		REG(m.R_M) = res;
		...
		swaddr_t addr;
		int len = read_ModR_M(eip + 1, &addr);
		DATA_TYPE res = MEM_R(addr) + REG(m.reg) + eflags.CF;
		MEM_W(addr,res);
...
make_helper(concat(adc_rm2r_, SUFFIX)) {
	...
	m.val = instr_fetch(eip + 1, 1);
	if(m.mod == 3) {
		DATA_TYPE res = REG(m.reg) + REG(m.R_M) + eflags.CF;
		REG(m.reg) = res;
		...
		swaddr_t addr;
		int len = read_ModR_M(eip + 1, &addr);
		DATA_TYPE res = REG(m.reg) + MEM_R(addr) + eflags.CF;
		REG(m.reg) = res;

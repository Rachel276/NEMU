...
#define mflags(rst,lhs,imm);\
	...
	eflags.ZF = (rst == 0);\
	eflags.CF = (lhs > rst) || (rst < lhs +eflags.CF);\
	eflags.OF = (MSB(lhs) == MSB(imm)) && (MSB(lhs) != MSB(rst));\
	eflags.PF = 0;\
	while (rst){\
		eflags.PF = !eflags.PF;\
		rst = rst & (rst - 1);\
...
make_helper(concat(adc_rm2r_, SUFFIX)) {
	...
	if(m.mod == 3) {
		DATA_TYPE rst = lhs + REG(m.R_M) + eflags.CF;
		...
		swaddr_t addr;
		int len = read_ModR_M(eip + 1, &addr);
		DATA_TYPE rst = lhs + MEM_R(addr) + eflags.CF;

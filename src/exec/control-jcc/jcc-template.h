...
make_helper(concat(ja_rel_, SUFFIX)) {
		...
		if (eflags.CF == 0 &&  eflags.ZF == 0)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}

make_helper(concat(jae_rel_, SUFFIX)) {
		DATA_TYPE_S rel = instr_fetch(eip + 1, DATA_BYTE);
		print_asm("jae" str(SUFFIX) " %x", eip + rel + 1 + DATA_BYTE);
		if (eflags.CF == 0)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jb_rel_, SUFFIX)) {
		...
		if (eflags.CF == 1)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jbe_rel_, SUFFIX)) {
		...
		if (eflags.CF == 1 ||  eflags.ZF == 1)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
//		printf("%d\n",eflags.ZF);
		if (eflags.ZF == 1)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jg_rel_, SUFFIX)) {
		...
		if (eflags.ZF == 0 && eflags.SF == eflags.OF)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jge_rel_, SUFFIX)) {
		...
		if (eflags.SF == eflags.OF)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jle_rel_, SUFFIX)) {
		...
		if (eflags.ZF == 1 || eflags.SF != eflags.OF)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jne_rel_, SUFFIX)) {
		...
		if (eflags.ZF == 0)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jno_rel_, SUFFIX)) {
		...
		if (eflags.OF == 0)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jnp_rel_, SUFFIX)) {
		...
		if (eflags.PF == 0)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jns_rel_, SUFFIX)) {
		...
		if (eflags.SF == 0)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jo_rel_, SUFFIX)) {
		...
		if (eflags.OF == 1)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jp_rel_, SUFFIX)) {
		...
		if (eflags.PF == 1)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(js_rel_, SUFFIX)) {
		...
		if (eflags.SF == 1)return rel + 1 + DATA_BYTE;
		else return DATA_BYTE + 1;
}

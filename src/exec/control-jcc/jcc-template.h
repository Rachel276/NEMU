...
make_helper(concat(ja_rel_, SUFFIX)) {
		...
		if (eflags.CF == 0 &&  eflags.ZF == 0)return rel + 2;
...
make_helper(concat(jbe_rel_, SUFFIX)) {
		...
		if (eflags.CF == 1 ||  eflags.ZF == 1)return rel + 2;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jg_rel_, SUFFIX)) {
		...
		if (eflags.ZF == 0 && eflags.SF == eflags.OF)return rel + 2;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jge_rel_, SUFFIX)) {
		...
		if (eflags.SF == eflags.OF)return rel + 2;
		else return DATA_BYTE + 1;
}
...
make_helper(concat(jle_rel_, SUFFIX)) {
		...
		if (eflags.ZF == 1 || eflags.SF != eflags.OF)return rel + 2;
		else return DATA_BYTE + 1;
}
